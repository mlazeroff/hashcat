...
KERNEL_FQ void m23000_init (KERN_ATTR_TMPS_ESALT (pbkdf2_sha1_tmp_t, pbkdf2_sha1_t))
{
  /**
   * base
   */
...
KERNEL_FQ void m23000_loop (KERN_ATTR_TMPS_ESALT (pbkdf2_sha1_tmp_t, pbkdf2_sha1_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void m23000_comp (KERN_ATTR_TMPS_ESALT (pbkdf2_sha1_tmp_t, pbkdf2_sha1_t))
{
  /**
   * base
   */
  ...
  sha1_hmac_init (&hmac_ctx, o, 20);

  u32 key[16] = {0};    // SCRAM defined key of "Client Key"
  key[0] = 0x436c6965;
  key[1] = 0x6e74204b;
  key[2] = 0x65790000;

  sha1_hmac_update (&hmac_ctx, key, 10);

  sha1_hmac_final (&hmac_ctx);

  o[0] = hmac_ctx.opad.h[0];
  o[1] = hmac_ctx.opad.h[1];
  o[2] = hmac_ctx.opad.h[2];
  ...
  const u32 r2 = sha_ctx.h[DGST_R2];
  const u32 r3 = sha_ctx.h[DGST_R3];
